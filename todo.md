...
### שורת התראות דחופות
בראש לוח הבקרה תופיע שורה בולטת (בצבע אדום/כתום) שמציגה דברים שדורשים טיפול מיידי.

**סוגי התראות:**
1. עבודות באיחור - עברו את מועד האספקה המובטח (אדום)
2. הצעות ממתינות - מחכות לאישור לקוח יותר מ-3 שעות (כתום)
3. ספקים לא אישרו - עבודות שנשלחו לספק ולא אושרו תוך 24 שעות (כתום)

**התנהגות:**
- השורה מופיעה רק כשיש בעיות (אם אין בעיות - לא מוצגת)
- לחיצה על התראה מגדילה את השדה ומציגה רשימה מפורטת
- כפתור "הסתר התראה זו ל-24 שעות" - התראה תעלה שוב אם לא טופלה אחרי 24 שעות
- אם הבעיה נפתרה במערכת (למשל עבודה נמסרה) - ההתראה נעלמת אוטומטית
- צליל עדין כשמופיעה התראה חדשה

- [ ] API endpoint לשליפת התראות דחופות
- [ ] טבלת dismissed_alerts לשמירת התראות שהוסתרו זמנית
- [ ] רכיב UI להצגת התראות בראש לוח הבקרה
- [ ] הגדלת שדה בלחיצה עם רשימה מפורטת
- [ ] צליל עדין להתראות חדשות

### תיקון פונקציות KPI בלוח בקרה
4 השדות העליונים (הצעות מחיר, לקוחות פעילים, הכנסות, שיעור המרה) משתמשים בפונקציות ישנות שלא מושכות דאטה נכונה. צריך לעדכן אותן לשלוף מ-supplier_jobs עם סטטוס delivered כמו שעשינו באנליטיקס.
- [ ] תיקון getKPIs בשרת לשלוף מ-supplier_jobs
- [ ] חישוב נכון של הכנסות, לקוחות פעילים, ושיעור המרה
