...
export default function PickedUp() {
  ...
  // Get all jobs and filter for picked_up status
  const { data: allJobs, isLoading, refetch } = trpc.jobs.list.useQuery();
  
  // Filter for picked_up status
  const jobs = allJobs?.filter((job: Job) => job.status === "picked_up") || [];
  ...
  const handleMarkDelivered = (jobId: number) => {
    updateStatus.mutate({
      jobId,
      status: "delivered",
      notifyCustomer: true,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">עבודות שנאספו מהספק ובדרך ללקוח</p>
        </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">בדרך ללקוח</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  ₪{filteredJobs.reduce((sum: number, j: Job) => sum + Number(j.totalJobPrice || j.quantity * Number(j.pricePerUnit) || 0), 0).toLocaleString()}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-amber-600">
                  {new Set(filteredJobs.map((j: Job) => j.customerName)).size}
                </p>
              </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-medium">אין עבודות בדרך</h3>
              <p className="text-muted-foreground mt-1">כל העבודות שנאספו כבר נמסרו ללקוחות</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">תאריך איסוף</TableHead>
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground">
                        <div className="flex items-center gap-1">
                          ...
                          {job.pickedUpAt ? new Date(job.pickedUpAt).toLocaleDateString("he-IL") : "-"}
                        </div>
                      </TableCell>
                      <TableCell>
                        <Button
                          ...
                          <div className="p-6 space-y-6">
                            ...
                            <div className="p-4 rounded-lg bg-gray-50 border border-gray-200">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                  <p className="text-sm text-muted-foreground">תאריך איסוף</p>
                                  <p className="font-medium text-blue-600">
                                    {jobDetails.pickedUpAt ? new Date(jobDetails.pickedUpAt).toLocaleDateString("he-IL") : "-"}
                                  </p>
                                </div>
                              </div>
                            </div>
